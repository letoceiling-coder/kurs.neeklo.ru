...
/** Гарантировать явные формулировки объекта/предмета/цели/задач/гипотезы во введении. */
function ensureIntroStructure(paras, outline, research) {
  ...
  if (!/предмет[\s\S]{0,30}исследован/i.test(text)) {
    inserts.push(`Предметом исследования является ${outline.subject || 'педагогические условия и методика развития познавательного интереса дошкольников'}.`);
  }
  if (!/цел[\s\S]{0,20}(работ|исследован)/i.test(text)) {
    inserts.push(`Целью работы является ${outline.goal || 'теоретическое обоснование и экспериментальная проверка эффективности педагогического средства'}.`);
  }
  if (!/задач/i.test(text) && outline.tasks?.length) {
    const tasks = outline.tasks.map((t, i) => `${i + 1}) ${t}`).join('; ');
    inserts.push(`Для достижения цели поставлены следующие задачи: ${tasks}.`);
  }
  if (isPedagogyDomain(research?.domain) && outline.hypothesis && !/гипотез/i.test(text)) {
    inserts.push(`Гипотеза исследования заключается в том, что ${outline.hypothesis}.`);
  }
  if (!inserts.length) return paras;
  console.log(`[quality] intro structure patched: ${inserts.length} block(s)`);
  return [...inserts, ...paras];
}
...
export async function generateDocument(params, onProgress = () => {}) {
  ...
  const cleaned = sanitizeTableBlocks(blocks);
  blocks.length = 0;
  blocks.push(...cleaned);
